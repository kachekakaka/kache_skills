...
上下文时保持只读，向用户报告证据和待决边界，不假定或自动调用任何外部 Skill。
...
只有存在项目专属术语时创建，固定在根目录，由用户另行选择的语义裁决流程维护。骨架 Skill 只检查
位置和入口，不检查或修改内容。
...
完整、长期真源是否收口，由 `$project-doc-consistency check` 检查；存在真实取舍时交由用户另行显式裁决。
...
职责属于语义决定，交由用户另行显式裁决；确认后再由本骨架实施结构变化。
...
- 术语、`CONTEXT.md`、ADR，以及合并、拆分、归档、删除和新承接职责等语义取舍由用户另行显式选择
  裁决方式。环境中已经提供 `$grill-with-docs` 时可以选择它；未提供时直接列出待决问题，不形成依赖。

需要真实取舍时，由用户显式组织“一致性检查提供证据 → 语义裁决 → 骨架实施结构变化”。交给本骨架的
交接只包含已确认决定和待实施动作类别；外部裁决完成不表示已经选择本骨架，也不携带修改授权。用户显式
选择并调用本骨架后，才实施结构变化。不需要真实取舍的明确事实修订可以由
`$project-doc-consistency sync` 在独立授权后完成。各次调用不得自动触发或继承修改授权。
