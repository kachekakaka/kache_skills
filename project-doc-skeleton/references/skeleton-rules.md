...
只有用户确认或 `$audit-project-docs` 已提供证据，证明其同时满足长期有效、职责独立、现有真源无法
...
完整、长期真源是否收口，由 `$audit-project-docs` 审计；存在真实取舍时交给 `$grill-with-docs`。
...
`$audit-project-docs` 只提供过期、重复、冲突和当前承接候选的事实证据。归档价值、合并、删除和新承接
...
`$audit-project-docs`。一对一迁移保持事实正文原样。
...
- 当前事实、过期内容、重复真源、正文职责越界和候选现有承接文档由 `$audit-project-docs` 处理。
- 术语、`CONTEXT.md`、ADR，以及合并、拆分、归档、删除和新承接职责等语义取舍由
  `$grill-with-docs` 处理。

交接顺序是“审计提供证据 → `$grill-with-docs` 裁决真实取舍 → 骨架实施结构变化”。不需要真实取舍的
明确事实修订可以由审计在独立授权后完成。三个 Skill 都必须显式调用；一个 Skill 只能建议另一个，
不得自动触发或继承修改授权。
