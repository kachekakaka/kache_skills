...
- 骨架 Skill 不持续审计或同步产品事实；为判断既有治理能否原样保留，可以读取直接相关的治理与安全证据。一致性 Skill 不创建、移动、重命名或删除文件，也不修改可执行测试代码。
- [ADR-0002](0002-prove-equivalent-governance-from-current-evidence.md) 补充了两个 Skill 各自独立证明治理等价的要求，但不改变结构实施与内容同步仍然分离的决定。
