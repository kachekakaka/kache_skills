...
- 输入：当前标准骨架工作区的活动 Markdown、归档索引和固定路径。
...
门禁机械检查标准骨架的固定路径与大小写、UTF-8 与 LF、普通相对链接和标题锚点、必要入口、两跳可达性、待办与方案生命周期、四列测试 Registry 及归档登记。`待确认` 待办可以没有方案或链接一份方案，`实施中` 待办必须链接一份方案，其他活动状态不得保留方案；任一待办最多一份。根 README 与 `docs/README.md` 直接链接至少三份相同专题 Markdown 时给出重复导航 warning。它不判断正文事实、生命周期语义、测试设计质量或非标准成熟项目是否结构等价。
